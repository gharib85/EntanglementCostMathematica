...
if verLessThan('matlab', '8.3')
    ...
    addParameter(parser, 'TOL', eps, @isscalar);
end
...
if(num_ops == 1 || max(p) >= 1) % of course we can distinguish 1 object
    ...
    cvx_begin sdp quiet
        ...
        subject to
            ...
            for j = 1:num_ops
%                 P(:,:,j) == semidefinite(16);
%                 partial_transpose([1 1 0 0],P(:,:,j)) == semidefinite(16);
                paren(Swap(partial_transpose([1 1 0 0],P(:,:,j)),[2,3], [2,2,2,2]),1,1) == paren(Swap(partial_transpose([1 1 0 0],P(:,:,j)),[2,3], [2,2,2,2]),16,16)
